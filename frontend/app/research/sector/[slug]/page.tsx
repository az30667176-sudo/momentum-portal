...
export function generateStaticParams() {
  return getAllSlugs('sector').map((slug) => ({ slug }))
}

export function generateMetadata({ params }: { params: { slug: string } }) {
  const issue = getIssue('sector', params.slug)
  if (!issue) return { title: '產業分享 | Sector Pulse' }
  return {
    title: `${issue.title} | 產業分享`,
  }
}
    ...
    <article>
      ...
      <header className="mb-10 pb-6 border-b border-gray-200">
        ...
        <p className="mt-3 text-sm text-gray-500">
          {issue.date} · 快照日 {issue.snapshotDate}
        </p>
      </header>
      ...
      <footer className="mt-16 pt-6 border-t border-gray-200 text-xs text-gray-400">
        ...
        公開資訊所做的產業研究紀錄，不構成任何投資建議。所有數據截至 {issue.snapshotDate}。
      </footer>
    </article>
  )
}
